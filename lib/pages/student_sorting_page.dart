...
class _StudentSortingPageState extends State<StudentSortingPage> {
  ...
  void _applySort() {
    ...
    final byScore = List<_RankedStudent>.from(list)..sort((a, b) => b.average.compareTo(a.average));
    ...
    setState(() {
      _ranked = list.map((e) => e.copyWith(localRank: localRank[e.student.id] ?? 0)).toList();
    });
  }

  String _noteFor(_RankedStudent row) {
    if (row.schoolRank != null && row.schoolRank! <= 3) {
      return 'أعلى 3 مدرسة #${row.schoolRank}';
    }
    if (row.schoolRank != null) {
      return 'مدرسة #${row.schoolRank}';
    }
    return '—';
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final tableWidth = maxW < 720 ? 720.0 : maxW;
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              // Title + actions (wraps, no overflow)
              Wrap(
                spacing: 10,
                runSpacing: 10,
                      ...
                      children: <Widget>[
                        SizedBox(
                          ...
                          child: DropdownButtonFormField<String>(
                            ...
                            isExpanded: true,
                            hint: const Text('كل الصفوف', overflow: TextOverflow.ellipsis),
                            items: _availableGrades
                                .map((g) => DropdownMenuItem(value: g, child: Text('الصف $g', overflow: TextOverflow.ellipsis)))
                                .toList(),
                            onChanged: (v) {
                              setState(() {
                                _selectedGrade = v ?? '';
                        ...
                        if (_sortMode == 'grade_section')
                          SizedBox(
                            ...
                            child: DropdownButtonFormField<String>(
                              ...
                              isExpanded: true,
                              hint: const Text('كل الشعب', overflow: TextOverflow.ellipsis),
                              items: _availableSections
                                  .map((s) => DropdownMenuItem(value: s, child: Text('شعبة $s', overflow: TextOverflow.ellipsis)))
                                  .toList(),
                              onChanged: (v) {
                                setState(() {
                                  _selectedSection = v ?? '';
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    // Table (scrolls horizontally only when needed; stays inside card)
                    Scrollbar(
